...
class FilialService {
  criarFilialNoMongo = async (filial: Filial) => {
    return await FilialSchema.create(filial)
      .then((filialCriada) =>
        Util.cadastro(filialCriada, `filial ${filial.codigo} criada no mongo`)
      )
  ...
  todasFiliais = async () => {
    ...
    return await Oracle.conectar()
      ...
      .then((resultadosDoSql) =>
        this.converterDadosSqlParaModelo(resultadosDoSql)
      )
